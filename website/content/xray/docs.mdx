---
title: X-Ray integration - Documentation
description: Documentation for the Bref X-Ray integration for PHP on AWS Lambda.
---

import Breadcrumbs from '../../src/components/Breadcrumbs';
import Image from 'next/image';
import transactionSearchEnable from './transaction-search-enable.png';
import transactionSearchSettings from './transaction-search-settings.png';
...
- Classic X-Ray traces load fast, while Transaction Search can take a few seconds.
- Transaction Search is enabled for the whole region. Check whether another team or tool depends on classic X-Ray.

The change is reversible. To enable Transaction Search ([official guide](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Enable-TransactionSearch.html)):

1. Open [CloudWatch](https://console.aws.amazon.com/cloudwatch/) in the right region.

    <Image className="mt-3 rounded-lg shadow-md border" src={transactionSearchEnable} alt="Enable Transaction Search in the AWS CloudWatch console" />
2. Go to **Transaction Search** and click **Enable Transaction Search**.
...
AWS X-Ray is a paid service. You can find the pricing details on the [AWS CloudWatch pricing page](https://aws.amazon.com/cloudwatch/pricing/).

- Classic X-Ray: $5 per million traces (100k free per month).
- Transaction Search: $0.35/GB of trace data stored → roughly $5 per million traces for a typical PHP app (though it depends on the size of each trace).

One trace is one Lambda invocation, i.e. one request or one job. X-Ray applies a sampling rate out of the box, so you will not be charged for all traces:

> The sampling rate is 1 request per second and 5 percent of additional requests.

That sampling applies to X-Ray classic _and_ Transaction Search, no difference.

This is quite useful: when an application has low traffic, most requests/invocations will be traced. However, when traffic ramps up, X-Ray costs don't scale linearly with traffic because only a percentage of requests will be traced. Enabling X-Ray in dev and staging environments can also be a great way to debug and troubleshoot issues with almost no costs.
