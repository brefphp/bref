...
function SponsorsList({ sponsors }) {
    const sortedSponsors = sponsors
        ...
        .map(sponsor => {
            ...
            // GitHub allows website URLs without a protocol (e.g. `example.com`),
            // which the browser would treat as relative links
            const websiteUrl = sponsor.sponsorEntity.websiteUrl;
            if (websiteUrl && !/^https?:\/\//.test(websiteUrl)) {
                sponsor.sponsorEntity.websiteUrl = `https://${websiteUrl}`;
            }
            return sponsor;
        })
