---
asIndexPage: true
description: "PHP runtimes for PHP on AWS Lambda, provided by the Bref open-source project."
---
...
The runtimes are available as AWS Lambda layers and as Docker images, [as explained below](#lambda-layers-or-docker-images).
...
This runtime works great to create **event-driven microservices**.
...
## Lambda layers or Docker images?

Bref runtimes are distributed in two forms:

- **AWS Lambda layers**: this is the default. Your code is deployed as a zip archive and the Bref runtime is added to the function as a Lambda layer. This is what `runtime: php-84` does in `serverless.yml`, as explained in the [Usage](#usage) section below and in [Runtimes in detail](./runtimes/runtimes-details.mdx).
- **Docker images**: the same runtimes are also published as Docker images (`bref/php-84`, `bref/php-84-dev`, etc.). Use them to [deploy container images to Lambda](./deploy/docker.mdx) instead of zip archives, or to [run your application locally](./local-development.mdx).

If you are getting started, use Lambda layers: it is simpler and faster. Docker is useful when your code exceeds the 250MB limit or when you need custom system binaries.
...
This plugin is what makes `runtime: php-84` work (as well as other utilities). It is explained in more detail in the section below.
...
The `${...}` notation is the [syntax to use variables](https://github.com/oss-serverless/osls/blob/4.x/docs/guides/variables.md) in `serverless.yml`. The Bref plugin provides the following variables:
