...
AWS Lambda can react to HTTP requests via [Lambda Function URLs](https://docs.aws.amazon.com/lambda/latest/dg/lambda-urls.html). On Lambda, the Bref runtime starts PHP-FPM and forwards HTTP Lambda events to PHP-FPM via the FastCGI protocol.
...
To deploy an HTTP application on AWS Lambda, use the `php-xx-fpm` runtime and combine it with the `url: true` option:
...
The `url` option will create a public HTTP endpoint that invokes the PHP code on each request.
...
If you use API Gateway, some integrations can add information to the HTTP request via the *request context*.
