---
description: "Run serverless event-driven PHP functions on AWS Lambda using Bref."
---
...
The `Context` object is inspired by the [`context` parameter in other languages](https://docs.aws.amazon.com/lambda/latest/dg/nodejs-prog-model-context.html) and provides information about the current lambda invocation (the request ID, the X-Ray trace ID, etc.).
...
A PHP function can be triggered manually from the CLI using the [`serverless invoke` command](https://github.com/oss-serverless/serverless/blob/main/docs/cli-reference/invoke.md):
...
> [!NOTE]
>
> The `aws/aws-sdk-php` package is very large (100MB+). To avoid hitting Lambda's 250MB size limit, [remove unused AWS services from the package](/docs/deploy.mdx#reducing-package-size).
