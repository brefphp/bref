---
description: Learn how to create serverless cron functions with PHP on AWS Lambda.
---
...
Cron events can be used to run CLI commands with the [Console runtime](../runtimes/console.mdx).
...
Read more about the options for the `schedule` event in the [Serverless documentation](https://github.com/oss-serverless/serverless/blob/main/docs/events/schedule.md).

## Cron functions

On top of running CLI cron tasks with the `php-xx-console` runtime, we can also run **event-driven functions** (using the [PHP function runtime](/docs/runtimes/function.mdx)) as cron tasks.
