...
The S3 bucket will automatically be created on deployment. You can listen to an existing S3 bucket via [the `existing: true` option](https://github.com/oss-serverless/osls/blob/4.x/docs/events/s3.md#using-existing-buckets).
...
Learn more about all the options available for S3 in `serverless.yml` [in the Serverless Framework documentation](https://github.com/oss-serverless/osls/blob/4.x/docs/events/s3.md).
