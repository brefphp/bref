...
Bref is designed out of the box to deploy using [`serverless.yml`](https://github.com/oss-serverless/osls).

Bref can also work with any other deployment tool: Terraform, CloudFormation, SAM, [AWS CDK](https://github.com/brefphp/constructs), Pulumi… However, the documentation and user experience are optimized for `serverless.yml`.
...
You can [learn more about that configuration format here](environment/serverless-yml.mdx).

## Learn more

Read more about `serverless deploy` in [the official documentation](https://github.com/oss-serverless/osls/blob/4.x/docs/guides/deploying.md).
