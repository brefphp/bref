---
description: "Run Laravel with Octane on AWS Lambda using Bref."
---


# Laravel Octane

To run the HTTP application with [Laravel Octane](https://laravel.com/docs/octane) instead of PHP-FPM, change the following options in the `web` function:

```yml filename="serverless.yml" {3-4,6}
functions:
    web:
...
- Octane keeps Laravel booted in a long-running process, [beware of memory leaks](https://laravel.com/docs/octane#managing-memory-leaks).
...
```sql
ALTER DATABASE my_database SET idle_in_transaction_session_timeout = '10000'; -- 10 seconds in ms
```
