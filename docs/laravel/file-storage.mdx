...
Make sure `serverless-lift` is listed in the `plugins` section of `serverless.yml` (the file generated for Laravel already includes it). Then use [the `storage` construct](https://github.com/getlift/lift/blob/master/docs/storage.md) in `serverless.yml`:
