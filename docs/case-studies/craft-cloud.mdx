...
The Craft Cloud API builds the Lambda container images using [Dagger](https://dagger.io), a Go-based Dockerfile alternative, and a custom Go application that orchestrates and inspects the builds. These builds run on ECS (Fargate), as AWS Lambda cannot build Container images directly.
...
In each environment, Craft CMS customer projects have automatic access to a MySQL or PostgreSQL database, S3 for storing files, a job queue preconfigured with SQS, cron tasks, and logs storage in CloudWatch. Customers can also choose the AWS regions in which their application will run.
...
With this architecture, Craft Cloud has served over **280 million HTTP requests and 5TB of data** in the last 30 days.
