---
description: "Learn how to store data and files in serverless PHP applications running on AWS Lambda."
---
...
Instead, data can be stored in [databases](/docs/environment/database.mdx) or in storage services like AWS S3.

### S3 storage

It is possible to deploy an S3 bucket in `serverless.yml` using the [`Storage` feature of the Lift plugin](https://github.com/getlift/lift/blob/master/docs/storage.md). For example:
...
Some frameworks allow pre-generating some caches. For example in Symfony the container can be compiled via `bin/console cache:warmup`, or in Laravel the config cache can be generated before deploying.
...
Like any service, DynamoDB tables can be deployed via CloudFormation using the `resources` key in `serverless.yml`:
