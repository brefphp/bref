...
- **Environments**: manage dev, staging and production across AWS accounts and regions from one dashboard, with the history of every deployment (also available from the terminal with [`bref deployments`](./cloud-deploy.mdx#deployment-history)).

### Monitor

- **Overview**: a diagram of each environment's architecture with live metrics on every component.
- **Logs**: search logs or tail them in real time in the dashboard, or search them from the terminal with [`bref logs`](/docs/environment/logs#reading-logs).
...
AI agents like Claude Code, Cursor or Codex can read the logs and the deployments of your applications with the `bref` CLI: `bref logs`, `bref deployments`, `bref deployments:show` and `bref deployments:logs`. For example, an agent can look for the error behind a bug report, or find why a deployment failed.

The CLI detects when an agent runs it and adapts its output: no colors or spinners, logs as compact text, and deployments as JSON.
