...
# AWS provides PHP through amazon-linux-extras for ARM architecture. Currently we have no other distribution
# we can use to install PHP on ARM.
RUN yum install -y amazon-linux-extras && amazon-linux-extras enable php7.4

# Bref needs cURL to make requests to the Lambda Runtime API. We also install php-sodium
# because Bref distributes PHP with sodium enabled by default.
# TODO: do we still need unzip?
RUN yum install -y curl php-cli php-sodium unzip

# These files are included on Amazon Linux 2 by default and we don't need to increase
# the layer size by copying them. However, we keep them here because it facilitates
# maintenance by knowing which files we expect to have on the layer.
...
# Default Extensions
RUN cp /usr/lib64/php/modules/json.so /bref/php-extensions/json.so
