FROM public.ecr.aws/lambda/provided:al2-arm64 as base
...
# PHP Binary
RUN /bin/cat /usr/bin/php > /bref/bin/php

RUN chmod +x /bref/bin/php

# Bref itself doesn't work without curl and json extensions
RUN /bin/cat /usr/lib64/php/modules/curl.so > /bref/php-modules/curl.so

# PHP 7.4 doesn't have json enabled by default.
RUN /bin/cat /usr/lib64/php/modules/json.so > /bref/php-modules/json.so
...
RUN /bin/cat /lib64/libncurses.so.6 > /bref/lib/libncurses.so.6
...
RUN /bin/cat /lib64/ld-linux-aarch64.so.1 > /bref/lib/ld-linux-aarch64.so.1
...
RUN /bin/cat /usr/lib64/php/modules/sodium.so > /bref/php-modules/sodium.so

RUN /bin/cat /usr/lib64/php/modules/ctype.so > /bref/php-modules/ctype.so
RUN /bin/cat /usr/lib64/php/modules/exif.so > /bref/php-modules/exif.so
RUN /bin/cat /usr/lib64/php/modules/fileinfo.so > /bref/php-modules/fileinfo.so
RUN /bin/cat /usr/lib64/php/modules/ftp.so > /bref/php-modules/ftp.so
RUN /bin/cat /usr/lib64/php/modules/gettext.so > /bref/php-modules/gettext.so
RUN /bin/cat /usr/lib64/php/modules/iconv.so > /bref/php-modules/iconv.so
RUN /bin/cat /usr/lib64/php/modules/tokenizer.so > /bref/php-modules/tokenizer.so
