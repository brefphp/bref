...
RUN yum install -y php81-php unzip
...
# PHP Binary
RUN /bin/cat /opt/remi/php81/root/usr/bin/php > /bref/bin/php

RUN chmod +x /bref/bin/php

# Modules
RUN /bin/cat /opt/remi/php81/root/lib64/php/modules/curl.so > /bref/php-modules/curl.so
...
RUN /bin/cat /opt/remi/php81/root/lib64/php/modules/sockets.so > /bref/php-modules/sockets.so
RUN /bin/cat /opt/remi/php81/root/lib64/php/modules/tokenizer.so > /bref/php-modules/tokenizer.so
